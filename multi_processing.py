...
words_queue = Queue(maxsize=1000)
out_queue = Queue()
# max_process = 4
...
def start_password_cracker():
    ...
    for my_password in crack_list:
        ...
        proc_read = []
        proc_read_file = Process(target=read_wordlist, args=())
        proc_read.append(proc_read_file)
        proc_read_file.start()

        # file_reader_thread = Thread(target=read_wordlist)
        # my_threads.append(file_reader_thread)
        # file_reader_thread.start()

        procs = []
        for i in range(max_process):
            p = Process(target=hash_processor, args=(algo, my_salt, my_hash))
            procs.append(p)
            p.start()
...
def read_wordlist():
    global wordlist_default
    with open(wordlist_default, 'r') as f:
        for word_s in f:
            word = word_s.rstrip('\n').strip()
            if word.isspace():
                continue
            words_queue.put(word)
            # print('put word in queue : {}'.format(word ))
...
def hash_processor(algo, my_salt, my_hash):
    # print('start hash processor')
